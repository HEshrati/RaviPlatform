...
import {
  BookOpen, Clock, ArrowLeft, Brain, Heart, Sparkles,
  Leaf, Zap, Star, Shield, Sun, RefreshCw,
} from "lucide-react";
...
interface Article {
  ...
  category: string; readTime: string; url: string; publishedAt?: string;
}
...
const CATEGORY_CONFIG: Record<string, { bg: string; border: string; text: string; icon: React.ReactNode }> = {
  ...
  "attachment": { bg: "rgba(249,115,22,0.1)", border: "rgba(249,115,22,0.25)", text: "#ea580c", icon: <Heart size={18} /> },
  "communication": { bg: "rgba(14,165,233,0.1)", border: "rgba(14,165,233,0.25)", text: "#0284c7", icon: <Sun size={18} /> },
  "emotion": { bg: "rgba(168,85,247,0.1)", border: "rgba(168,85,247,0.25)", text: "#9333ea", icon: <Brain size={18} /> },
};
...
export default function ArticlesPreviewSection() {
  ...
  useEffect(() => {
    // ✅ endpoint درست: از backend اصلی
    fetch(`${API_URL}/api/ai-content/articles?limit=9`)
      .then((r) => (r.ok ? r.json() : null))
      .then((res: any) => {
        // پشتیبانی از فرمت‌های مختلف response
        const raw: any[] = res?.items || res?.data || res?.articles || (Array.isArray(res) ? res : []);
        const mapped: Article[] = raw.map((a: any) => ({
          id: a.id,
          title: a.title,
  ...
  return (
    <section id="articles-section" className="py-10 md:py-16 px-4 md:px-6 relative overflow-hidden bg-transparent scroll-mt-24" dir="rtl">
      ...
      <div className="container mx-auto max-w-6xl relative z-10">
        ...
        <div className="flex items-end justify-between mb-8 md:mb-12">
          <div>
            ...
            <div className="flex items-center gap-3 mb-2">
              <h2 className="text-2xl md:text-3xl lg:text-4xl font-black text-slate-900">کتابخانه راوی</h2>
              <div className="w-9 h-9 md:w-11 md:h-11 rounded-2xl flex items-center justify-center flex-shrink-0"
                style={{ background: "linear-gradient(135deg, #FF6B00, #FF9A3C)", boxShadow: "0 4px 14px rgba(255,107,0,0.28)" }}>
            ...
            <p className="text-slate-500 text-sm md:text-base max-w-md">
              ...
              <span className="text-orange-500 font-semibold">{todayLabel}</span>
            </p>
          </div>
          <Link href="/article" className="hidden md:flex items-center gap-2 font-bold text-sm transition-colors group" style={{ color: "#FF7A00" }}>
            همه مقالات
          ...
          {loading
            ? [1, 2, 3].map((i) => (
                <div key={i} className="rounded-3xl overflow-hidden border animate-pulse" style={{ background: "white", borderColor: "rgba(0,0,0,0.06)" }}>
                  <div className="h-44" style={{ background: "rgba(255,107,0,0.05)" }} />
                  ...
                  <Link key={article.id} href={article.url}
                    ...
                    <div className="p-5">
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold mb-3"
                        style={{ background: cfg.bg, color: cfg.text, border: `1px solid ${cfg.border}` }}>
                        <span style={{ transform: "scale(0.72)" }}>{cfg.icon}</span>
                        {article.category}
                      </span>
                      <h3 className="text-slate-800 font-black text-base mb-2 line-clamp-1 group-hover:text-orange-600 transition-colors">{article.title}</h3>
        ...
        <div className="mt-8 text-center md:hidden">
          <Link href="/article" className="inline-flex items-center gap-2 px-6 py-3 rounded-2xl font-bold text-sm text-white transition-all hover:opacity-90 active:scale-95"
            style={{ background: "linear-gradient(135deg, #FF6B00, #FF9A3C)", boxShadow: "0 6px 20px rgba(255,107,0,0.28)" }}>
