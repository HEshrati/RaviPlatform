      ...
      <div className="rounded-3xl p-5 border border-slate-200"
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <p className="text-slate-300 text-xs">همنشینی‌های متناسب با شما</p>
          </div>
        </div>
        <div className="relative">
          <Search className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-900/40" size={16} />
          <input
            type="text"
            placeholder="جستجو در همنشینی‌ها..."
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            className="w-full rounded-xl pr-9 pl-4 py-2.5 text-sm text-slate-900 placeholder-white/40 outline-none focus:ring-2 focus:ring-orange-500"
            style={{ background: "rgba(0,0,0,0.06)", border: "1px solid rgba(255,255,255,0.12)" }}
          />
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.slice(0, 12).map((event) => (
            <Link href={`/events/${event.id}`} key={event.id} className="group block">
              <div className="rounded-2xl overflow-hidden border border-slate-200 hover:border-orange-500/30 transition-all hover:-translate-y-0.5"
                style={{ background: "rgba(0,0,0,0.02)" }}>
