...
export default function CompleteProfilePage() {
  ...
  useEffect(() => {
    ...
    // ادمین‌ها هم باید پروفایل کامل کنن
    const hasName = !!state.user?.name?.trim();
    const hasCity = !!(state.city || (state.user as any)?.city);
    const isTestTaken = !!state.user?.isTestTaken;
    if (hasName && hasCity && isTestTaken) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setDone(true);
      // اگه تست نداده → به تست، وگرنه → داشبورد
      const testDone = !!state.user?.isTestTaken;
      setTimeout(() => router.replace(testDone ? "/dashboard" : "/test"), 1800);
    } catch (e: any) {
