...
export default function EventsPage() {
  ...
  const filtered = (() => {
    ...
    // اولویت‌بندی بر اساس محله کاربر
    const userNeighborhood =
      (state.user as any)?.neighborhood ||
      (state.user as any)?.profile?.neighborhood ||
      "";
    if (userNeighborhood) {
      list = [...list].sort((a, b) => {
        const aMatch = a.location?.includes(userNeighborhood) ? -1 : 0;
        const bMatch = b.location?.includes(userNeighborhood) ? -1 : 0;
  ...
  return (
    ...
    >
      ...
      {/* ── هدر چسبان صفحه ── */}
      <div
        className="sticky top-16 z-30 border-b border-slate-100/30 shadow-sm"
        style={{
      ...
      >
        <div className="max-w-lg mx-auto px-4 pt-3 pb-2">
          <h1 className="text-center text-lg font-black text-slate-900 mb-3">
            رزرو همنشینی
          </h1>
          ...
          {userCity && (
            <div className="flex items-center gap-1 text-xs text-slate-500 pb-1">
              ...
              <span>
                ...
                {((state.user as any)?.neighborhood ||
                  (state.user as any)?.profile?.neighborhood) &&
                  ` · اولویت محله ${(state.user as any)?.neighborhood || (state.user as any)?.profile?.neighborhood}`}
              </span>
            </div>
          )}
        </div>
